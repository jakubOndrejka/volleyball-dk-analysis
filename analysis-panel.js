...
let coachName = "",
  ...
  saving = false;
try {
  coachName = localStorage.getItem(coachNameKey) || "";
} catch {}
const esc = (value) =>
...
let context = null,
  index = null,
  published = { version: 1, teams: {} },
  local = { version: 1, teams: {} },
  loadError = "",
  team = "",
  selectedMatch = "all",
  selectedSet = "",
...
try {
  ...
  if (saved) local = validateConfig(JSON.parse(saved));
} catch {
  loadError =
    "Saved setter choices could not be read. Import your exported configuration to restore them.";
}
const params = new URLSearchParams(location.search);
let firstContext = true;
function merged() {
  return shared.merge(published, local);
}
async function initialiseShared() {
  if (!sharedInit)
    sharedInit = (async () => {
      try {
        const config = await json("./shared-coaches-config.json");
        if (typeof config.endpoint !== "string")
          throw new Error(
            "shared-coaches-config.json needs an endpoint string.",
          );
        shared = new SharedCoaches(config.endpoint);
...
async function refreshShared() {
  if (saving) return;
  saving = true;
  const button = root.querySelector("#a-refresh-shared");
  if (button) {
    button.disabled = true;
    button.textContent = "Loading shared choices…";
  }
  try {
    if (!shared.enabled) {
      sharedInit = null;
      await initialiseShared();
    } else {
      await shared.refresh();
      sharedError = "";
    }
...
const serveColumns = [
  ["name", "Player"],
  ["servingTurns", "Serving turns"],
  ["firstRalliesWon", "First rallies won"],
  ["firstRallyWinPct", "First-rally win %", "pct"],
  ["zeroPointTurns", "Zero-point turns"],
  ["onePointTurns", "1 point"],
  ["twoPointTurns", "2 points"],
  ["threeToFourPointTurns", "3–4 points"],
  ["fivePlusPointTurns", "5+ points"],
  ["threePlusRunPct", "3+ run %", "pct"],
  ["bestPointRun", "Best point run"],
  ["sets", "Sets played"],
  ["matches", "Matches played"],
  ["serves", "Serves"],
  ["serveWinPct", "Serve win %", "pct"],
  ["servesPerSet", "Serves / set", "dec"],
  ["pointsPerTurn", "Points / turn", "dec"],
];
const rotationColumns = [
  ...
  ["netPointsPer100", "Net points / 100", "signed"],
  ["serves", "Serves"],
  ["serveWins", "Serve wins"],
  ["serveWinPct", "Serve win %", "pct"],
  ["received", "Received"],
  ["sideOuts", "Side-outs"],
  ["sideOutPct", "Side-out %", "pct"],
];
function table(rows, columns, label) {
  return `<div class="a-table-wrap" tabindex="0" role="region" aria-label="${esc(label)}; scroll horizontally for all columns"><table class="a-table"><thead><tr>${columns.map((c) => `<th scope="col">${esc(c[1])}</th>`).join("")}</tr></thead><tbody>${rows
    .map(
      (r) =>
        `<tr class="${r.rotation === "Unconfirmed" ? "a-unconfirmed" : ""}">${columns
          .map(([key, , type], i) => {
            const v = r[key];
            const text =
              type === "pct"
...
function entries() {
  return (index?.matches || []).filter(
    (m) =>
      +m.leagueId === +context?.leagueId && [m.home, m.away].includes(team),
  );
}
...
function renderSetup(draft = null) {
  const target = root.querySelector("#a-setup-content");
  if (!target) return;
  const ready = loaded;
  if (!ready.length) {
    target.innerHTML =
      '<p class="a-note">Setter selection becomes available when a match PDF has been read.</p>';
    return;
  }
  if (!ready.some((m) => m.id === setupMatch))
    setupMatch = (ready.find((m) => m.id === selectedMatch) || ready[0]).id;
  const match = ready.find((m) => m.id === setupMatch),
    ...
    previous = structuredClone(
      draft ||
        merged().teams[key]?.matches?.[matchId] || {
          setters: [],
          fallbacks: [],
          system: "single",
        },
...
<p class="a-note">${shared.enabled ? "Saving, clearing or restoring whole-match choices updates this team’s match for everyone. Other teams and matches are not changed." : "Saved on this browser. Export setter choices below to share them with teammates."} Libero exchanges are absent from the PDF; choose setters who stay in the regular rotation lineup.</p>`;
  ...
  if (nameInput)
    nameInput.oninput = () => {
      coachName = nameInput.value;
    };
  const codeInput = target.querySelector("#a-coach-code");
  if (codeInput) {
    codeInput.value = coachPassword;
    codeInput.oninput = () => {
      coachPassword = codeInput.value;
    };
  }
  const disable = (busy) => {
    target.querySelectorAll("button,input,select").forEach((el) => {
      el.disabled = busy;
    });
    if (
      !busy &&
  ...
  target.querySelector("#a-setup-set").onchange = (e) => {
  ...
  const write = async (clear = false, inherit = false) => {
    ...
    if (!clear)
      target.querySelectorAll("[data-shirt]").forEach((s) => {
        if (s.value)
          next[s.value === "setter" ? "setters" : "fallbacks"].push(
            s.dataset.shirt,
          );
      });
    if (setNumber) {
      matchChoices.sets ??= {};
      if (inherit) delete matchChoices.sets[setNumber];
    ...
    if (shared.enabled) {
      ...
      if (!coachName || !coachPassword) {
        message.textContent =
          "Enter your name and the shared coach code first.";
        (!coachName ? nameInput : codeInput).focus();
        return;
      }
      saving = true;
      disable(true);
      message.textContent = "Saving to Google… waiting for confirmation.";
      try {
        await shared.save({
          leagueId,
          team: matchTeam,
        ...
        // Shared storage is authoritative; discard this stale personal copy only.
        if (local.teams[key]) delete local.teams[key].matches[matchId];
        try {
          saveLocal();
          localStorage.setItem(coachNameKey, coachName);
        } catch {}
        saving = false;
        renderSetup();
        renderReport();
        const status = root.querySelector("#a-save-message");
        if (status) status.textContent = "Saved for everyone. Tables updated.";
      } catch (error) {
        message.textContent = error.message;
        conflicted = error.code === "CONFLICT";
        target.querySelector("#a-load-latest").hidden = false;
        ...
        saving = false;
        disable(false);
        // A league/team switch can replace the form while the request is in flight.
        if (!target.isConnected) renderSetup();
      }
      return;
    }
    local.teams[key] ??= { matches: {} };
    local.teams[key].matches[matchId] = matchChoices;
    try {
      saveLocal();
      renderSetup();
      root.querySelector("#a-save-message").textContent =
        "Saved. Tables updated.";
      renderReport();
    } catch {
      root.querySelector("#a-save-message").textContent =
        "Browser storage is unavailable. Export your choices now to keep them.";
      renderReport();
    }
  };
...
function renderReport() {
  ...
  const matches = loaded.filter(
    (m) => selectedMatch === "all" || m.id === selectedMatch,
  );
  if (!matches.length) {
    report = null;
    const current = entries().find((m) => m.id === selectedMatch);
    target.innerHTML = `<div class="a-empty"><h3>${current?.status === "review" ? "This sheet needs a closer look" : entries().length ? "Waiting for a readable scoresheet" : "The first whistle is still ahead"}</h3><p>${esc(current?.reason || "Finished matches appear here after their scorecard and roster PDFs are published and checked.")}</p>${current?.source?.scorecardUrl ? `<a href="${esc(safeUrl(current.source.scorecardUrl))}" target="_blank" rel="noopener">Open the scoresheet PDF ↗</a>` : ""}</div>`;
    return;
  }
  try {
    report = analyseTeam(matches, team, merged(), selectedSet || null);
  } catch (error) {
    target.innerHTML = `<div class="a-notice error">${esc(error.message)} This match is excluded until the data is corrected.</div>`;
    return;
  }
  const r = report,
    firstWins = r.serving.reduce((n, p) => n + p.firstRalliesWon, 0),
    turns = r.serving.reduce((n, p) => n + p.servingTurns, 0);
  const shown = r.serving
    .filter((p) => p.servingTurns >= minTurns)
    .sort(
      (a, b) =>
...
<section class="a-card"><div class="a-card-head"><div><h3>Make that first serve count.</h3><p>Runs and first-rally wins, player by player.</p></div><button id="a-serving-csv" class="a-small">Download serving CSV ↓</button></div><div class="a-toolbar"><label>Minimum turns <input id="a-min-turns" type="number" min="0" max="1000" value="${minTurns}"></label><label>Sort by <select id="a-sort">${[
    ["servingTurns", "Serving turns"],
    ["firstRallyWinPct", "First-rally win %"],
    ["threePlusRunPct", "3+ run %"],
    ["pointsPerTurn", "Points / turn"],
    ["servesPerSet", "Serves / set"],
  ]
    .map(
      ([v, t]) =>
...
<section class="a-card"><div class="a-card-head"><div><h3>Where do we get stuck?</h3><p>S1–S6 = setter position before the rally. Side-outs count in the receiving rotation, before the team rotates.</p></div><button id="a-rotation-csv" class="a-small">Download rotation CSV ↓</button></div><div class="a-bars">${r.rotation
  ...
  target.querySelector("#a-sort").onchange = (e) => {
    sort = e.target.value;
    renderReport();
  };
  target.querySelector("#a-serving-csv").onclick = () =>
    download(
      `${team}-serving-${selectedMatch}.csv`,
      csv(shown, serveColumns),
...
function render() {
  if (!context) return;
  const list = entries(),
    ready = list.filter((m) => m.status === "ready"),
    teams = context.teams.map((t) => t.name),
    completed = context.fixtures.filter(
      (f) =>
        f.score && f.completed !== false && [f.home, f.away].includes(team),
    ).length;
...
<div class="a-card"><div class="a-card-head"><div><h3>${shared.enabled ? "A backup, just in case." : "Keep everyone on the same page."}</h3><p>Export your setter choices, or import a previous backup.</p></div></div><div class="a-actions"><button id="a-export">Export setter choices ↓</button><button id="a-import">Import setter choices ↑</button><input id="a-import-file" type="file" accept=".json,application/json" hidden><span id="a-import-message" class="a-save-message" role="status"></span></div><p class="a-note">${shared.enabled ? "Shared choices take priority over older browser copies. Import only stages choices in this browser; it does not upload them. Select a match, use ‘Review my browser choices’ if shown, then Save for all coaches. No GitHub edits needed for normal coaching." : "To publish these choices for everyone, replace <code>data/analysis-config.json</code> in your repository with the exported file. Personal choices in this browser take precedence."}</p></div>`;
  root
    .querySelector("#a-refresh-shared")
  ...
  root.querySelector("#a-match").onchange = (e) => {
  ...
  root.querySelector("#a-set").onchange = (e) => {
    selectedSet = e.target.value;
    renderReport();
  };
  root.querySelector("#a-retry")?.addEventListener("click", () => {
    index = null;
    loadTeam();
  });
  root.querySelector("#a-export").onclick = () =>
    download("analysis-config.json", JSON.stringify(merged(), null, 2) + "\n");
  root.querySelector("#a-import").onclick = () =>
    root.querySelector("#a-import-file").click();
  root.querySelector("#a-import-file").onchange = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const message = root.querySelector("#a-import-message");
    try {
      if (file.size > 1000000)
        throw new Error("Configuration file is too large.");
      const incoming = validateConfig(JSON.parse(await file.text()));
      for (const [key, t] of Object.entries(incoming.teams)) {
        local.teams[key] ??= { matches: {} };
        Object.assign(local.teams[key].matches, t.matches);
      }
      saveLocal();
      renderSetup();
      renderReport();
      message.textContent = shared.enabled
        ? "Imported into this browser only. Review each match and Save for all coaches to share it."
        : "Imported. Tables updated.";
    } catch (error) {
...
async function loadTeam() {
  const token = ++request;
  root.innerHTML =
    '<div class="a-empty" role="status">Reading match analysis…</div>';
  try {
    if (!index) {
      const values = await Promise.all([
        json("./data/analysis/index.json"),
        json("./data/analysis-config.json"),
        initialiseShared(),
      ]);
      if (token !== request) return;
      index = values[0];
      published = validateConfig(values[1]);
    }
    const list = entries().filter((m) => m.status === "ready");
    const results = await Promise.allSettled(
      list.map((m) => {
        if (!/^matches\/\d+\.json$/.test(m.file))
          throw new Error("Invalid match data path.");
        return json(`./data/analysis/${m.file}`);
      }),
    );
    if (token !== request) return;
    loaded = results
      ...
      .map((r) => r.value);
    loadError = results.some((r) => r.status === "rejected")
      ? "Some match files could not be loaded. The available matches are shown."
      : "";
    if (
      selectedMatch !== "all" &&
      !entries().some((m) => m.id === selectedMatch)
    )
      selectedMatch = "all";
    render();
  } catch (error) {
...
function setContext(c) {
  ...
  if (changed || !c.teams.some((t) => t.name === team)) {
    team =
      ...
      c.team ||
      c.teams.find((t) => t.name.startsWith("KSV"))?.name ||
      c.teams[0]?.name ||
      "";
    if (!c.teams.some((t) => t.name === team)) team = c.teams[0]?.name || "";
    selectedMatch = firstContext ? params.get("match") || "all" : "all";
    selectedSet = "";
    setupSet = "";
...
window.addEventListener("volleyball-context", (e) => setContext(e.detail));
if (window.volleyballContext) setContext(window.volleyballContext);
const showView = (view) => {
  const analysis = view === "analysis";
  document.getElementById("results-view").hidden = analysis;
  root.hidden = !analysis;
  document.body.classList.toggle("analysis-active", analysis);
  for (const btn of document.querySelectorAll("[data-view]"))
    btn.setAttribute("aria-pressed", String(btn.dataset.view === view));
  history.replaceState(
    null,
    "",
    `${location.pathname}${location.search}${analysis ? "#analysis" : ""}`,
  );
};
document
  .querySelectorAll("[data-view]")
  .forEach((b) => (b.onclick = () => showView(b.dataset.view)));
showView(location.hash === "#analysis" ? "analysis" : "results");
